    ...
    >
      <div className="h-64 relative overflow-hidden bg-[#052553]/60">
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-[#031835] to-transparent">
          <h3 className="font-orbitron text-xl font-bold text-white">{member.name}</h3>
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {member.skills?.map((skill, i) => (
            <span key={i} className="px-2 py-1 bg-[#0A4080]/50 text-[#00F5FF] text-xs rounded-md">{skill}</span>
          ))}
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            {member.linkedinUrl && (
              <a href={member.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-white hover:text-[#00F5FF] transition-colors">
                <i className="fab fa-linkedin"></i>
              </a>
            )}
            {member.githubUrl && (
              <a href={member.githubUrl} target="_blank" rel="noopener noreferrer" className="text-white hover:text-[#00F5FF] transition-colors">
                <i className="fab fa-github"></i>
              </a>
            )}
            {member.twitterUrl && (
              <a href={member.twitterUrl} target="_blank" rel="noopener noreferrer" className="text-white hover:text-[#00F5FF] transition-colors">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="mt-6 text-lg max-w-3xl mx-auto text-gray-300/90">
            Meet our team of modern knights, each bringing unique skills and expertise to our quest of democratizing AI.
          </p>
        </motion.div>
